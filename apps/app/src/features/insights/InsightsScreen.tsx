...
import { MilkStash } from "@/features/milk";
...
export const InsightsScreen = observer(function InsightsScreen() {
  ...
  return (
    <View bg="paper" style={styles.fill}>
      {/* The stash writes are undoable like every other action, so this screen
          hosts its own Undo bar (Home has a separate one). */}
      <UndoProvider>
            ...
            >
              ...
              >
                <View
                  style={{
                    ...
                    backgroundColor: theme.color.accent.sleep,
                  }}
                />
              ...
              {prediction ? (
                <>
                  <Text
                    variant="display"
                    style={{ fontSize: theme.type.scale.title }}
                  >
                    {formatClockTime(prediction.start)} –{" "}
                    {formatClockTime(prediction.end)}
                  </Text>
                ...
                <>
                  ...
                  <Text variant="body" color="inkSoft">
                    Log a nap or two and a suggested sleep window will appear
                    here.
                  </Text>
                </>
              )}
              ...
              <View style={[styles.grid, { gap: theme.space.xl }]}>
                ...
                {summary.longestSleepSeconds > 0 ? (
                  <StatusStat
                    ...
                    value={formatDuration(summary.longestSleepSeconds)}
                    accent="sleep"
                  />
                ) : null}
                <StatusStat
                  label="naps"
                  value={String(summary.naps)}
                  accent="sleep"
                />
              </View>
            </View>
